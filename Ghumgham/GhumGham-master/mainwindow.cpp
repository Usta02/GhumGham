...
void MainWindow::homeScreen()
{
    ...
    head->setText("Welcome to GhumGham");
    head->setStyleSheet("font-size: 27pt; color: red;");
    head->setAlignment(Qt::AlignCenter);

    QLabel *logo = new QLabel();
    QPixmap logojpg(":/Images/img/ghumgham.jpg");
    ...
    setWindowTitle("GhumGham");

}

void MainWindow::findDistance() {
    qDebug() << "Enter pressed.";
    QString fr = goLocationFrom->text().toLower();
    QString t = goLocationTo->text().toLower();
    qDebug() << fr + " to " + t;
    distValue->clear();
    if ((fr == "kathmandu" && t == "pokhara") || (t == "kathmandu" && fr == "pokhara"))
        distValue->insert("200 KM");
    else if ((fr == "kathmandu" && t == "chitwan") || (t == "kathmandu" && fr == "chitwan"))
        distValue->insert("98 KM");
    else if ((fr == "kathmandu" && t == "annapurna base camp") || (t == "kathmandu" && fr == "annapurna base camp"))
        distValue->insert("168 KM");
    else if ((fr == "kathmandu" && t == "chitlang") || (t == "kathmandu" && fr == "chitlang"))
        distValue->insert("26.7 KM");
    else if ((fr == "kathmandu" && t == "jiri") || (t == "kathmandu" && fr == "jiri"))
        distValue->insert("186 KM");
    else if ((fr == "kathmandu" && t == "khaptad") || (t == "kathmandu" && fr == "khaptad"))
        distValue->insert("448 KM");
    else
        distValue->insert("No data available.");
}
...
void MainWindow:: mapScreen()
{
    ...
    bkgnd = bkgnd.scaled(1200,900, Qt::KeepAspectRatio);
    QPalette palette;
...
//                icons[i][j]->setStyleSheet("background-color: red;");
    ...
    goLocationFrom = new QLineEdit;
    goLocationTo = new QLineEdit;

    searchBar->setFixedSize(250,40);
    goLocationFrom->setFixedSize(250,40);
    goLocationFrom->setStyleSheet("font-family: Lato;");
    goLocationFrom->setStyleSheet("padding: 0px 5px;");
    goLocationFrom->setPlaceholderText("Fabulous starting location...");

    goLocationTo->setStyleSheet("font-family: Lato;");
    goLocationTo->setStyleSheet("padding: 0px 5px;");
    goLocationTo->setFixedSize(250,40);
    goLocationTo->setPlaceholderText("Your sweet destination...");


    distValue = new QLineEdit;
    distValue->insert("");
    distValue->setStyleSheet("font-family: Lato;");
    distValue->setStyleSheet("padding: 0px 5px;");
    distValue->setFixedSize(250,40);
    distValue->setReadOnly(true);
    QLabel *distLabel = new QLabel;
    distLabel->setText("Calculate distance");

    layout->addWidget(distLabel, 21, 1, 6, 1);

    connect(goLocationFrom, SIGNAL(returnPressed()), this, SLOT(findDistance()));
    connect(goLocationTo, SIGNAL(returnPressed()), this, SLOT(findDistance()));
    ...
    layout->addWidget(goLocationFrom, 21, 1, 10, 1);
    layout->addWidget(goLocationTo, 21, 1, 15, 1);
    layout->addWidget(distValue, 21, 1, 30, 1);
...
void MainWindow::about()
{
    QMessageBox::about(this, tr("About"),
            tr("Welcome to GhumGham."
               "This is a tourism based project developed by CS first year students at KU"));
}
...
{
    fileMenu = menuBar()->addMenu(tr("&GhumGham"));
